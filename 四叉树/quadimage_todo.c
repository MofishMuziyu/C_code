...
void readImage(int p, char *inFile, char *outFile) // Note that width == height here
{
	...
	image = (color **)malloc(width*sizeof(color*));
    for (i = 0; i < height; i++)
	{	
	...
	for (i = 0; i < height; i++)
	{
		...
		fread(image[i], sizeof(color), width, f);
	}
	fclose(f);

	struct Quadnode *root = (struct Quadnode *)malloc(width*sizeof(struct Quadnode));
	init_quadnode(root);
	Quadnode_create(image,width,height,0,0, root,p);
	printImage(outFile,width,image);
...
int main()
{
	...
	if(1)
	{
		...
		//inFile = argv[2];
		//outFile = argv[3];
		strcpy(inFile, "D:/desktop/code2/sichashu/a.ppm");
		strcpy(outFile, "D:/desktop/code2/sichashu/result.ppm");
