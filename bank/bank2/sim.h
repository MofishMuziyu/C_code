...
void Simulation::RunSimulation(void)
{
    ...
    while (!pq.PQEmpty())
    {
        ...
        else
        {
            ...
            if (jump_pq.PQEmpty()!=true && no_vip){//如果需要插队的队列里面有VIP在等,并且当前被插队的队列没有VIP
            ...
            for(num = 0; num < pq.PQLength(); num++){ //小于count
                ...
                if(event.GetTellerID() == tellerID && event.GetEventType() == departure){//对后面的顾客的离开事件进行修改，只需要修改等待时间
                     int  event_waittime = event.GetWaitTime() + vip_st;
                    newevent = Event(event.GetTime() + vip_st,event.GetEventType(),event.GetCustomerID(),
                    event.GetTellerID(), event_waittime,event.GetServiceTime(),event.Getvip());
            ...
            Event vip_depart = Event(e.GetTime() + vip_st, departure, vip_event.GetCustomerID(), tellerID, vip_wt, vip_st, true);
            cout<<"vip jump"<<"  customer :"<<vip_event.GetCustomerID()<<endl;
            pq.PQInsert(vip_depart);
            //cout<<"error vip depart"<<endl;
            }
